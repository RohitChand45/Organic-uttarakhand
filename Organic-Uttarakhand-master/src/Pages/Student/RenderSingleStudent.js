...
export default function RenderSingleStudent(props) { 
    const {row,deleteStudent,index}=props
    ...
    return (
        ...
        <tr>
            ...
            <td align="center">{row.present_add}</td>
            ...
            <td align="center">
                <button className="btn btn-danger m-1" onClick={()=> deleteStudent(row.id,index)}> Delete </button>
                <Link to={"/student/"+row.id}>
                <button className="btn btn-info m-1"  data-toggle="modal" data-target={"#exampleModalCentered"+index}> Edit </button>
                </Link>
            </td>
        </tr>
